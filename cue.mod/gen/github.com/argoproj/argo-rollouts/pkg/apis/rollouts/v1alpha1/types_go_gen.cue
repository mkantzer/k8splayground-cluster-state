...
#CanaryStrategy: {
	...
	minPodsPerReplicaSet?: null | int32 @go(MinPodsPerReplicaSet,*int32) @protobuf(16,varint,opt)
}
...
#RolloutTrafficRouting: {
	...
	traefik?: null | #TraefikTrafficRouting @go(Traefik,*TraefikTrafficRouting) @protobuf(7,bytes,opt)
	...
	// traffic router.
	managedRoutes?: [...#MangedRoutes] @go(ManagedRoutes,[]MangedRoutes) @protobuf(8,bytes,rep)

	// Apisix holds specific configuration to use Apisix to route traffic
	apisix?: null | #ApisixTrafficRouting @go(Apisix,*ApisixTrafficRouting) @protobuf(9,bytes,opt)

	// +kubebuilder:validation:Schemaless
	// +kubebuilder:pruning:PreserveUnknownFields
	// +kubebuilder:validation:Type=object
	// Plugins holds specific configuration that traffic router plugins can use for routing traffic
	plugins?: {...} @go(Plugins,map[string]json.RawMessage) @protobuf(10,bytes,opt)
}
...
#ApisixRoute: {
	...
	rules?: [...string] @go(Rules,[]string) @protobuf(2,bytes,rep)
}
...
#NginxTrafficRouting: {
	...
	// +optional
	stableIngresses?: [...string] @go(StableIngresses,[]string) @protobuf(4,bytes,rep)
}
...
#IstioVirtualService: {
	...
	tcpRoutes?: [...#TCPRoute] @go(TCPRoutes,[]TCPRoute) @protobuf(4,bytes,rep)
}
...
#RolloutExperimentTemplate: {
	...
	service?: null | #TemplateService @go(Service,*TemplateService) @protobuf(7,bytes,opt)
}
...
#CanaryStep: {
	...
	// +optional
	setHeaderRoute?: null | #SetHeaderRoute @go(SetHeaderRoute,*SetHeaderRoute) @protobuf(6,bytes,opt)

	// SetMirrorRoutes Mirrors traffic that matches rules to a particular destination
	// +optional
	setMirrorRoute?: null | #SetMirrorRoute @go(SetMirrorRoute,*SetMirrorRoute) @protobuf(8,bytes,opt)
}
...
#SetMirrorRoute: {
	...
	// +optional
	match?: [...#RouteMatch] @go(Match,[]RouteMatch) @protobuf(2,bytes,opt)
	...
	percentage?: null | int32 @go(Percentage,*int32) @protobuf(4,varint,opt)
}

#RouteMatch: {
	// Method What http methods should be mirrored
	// +optional
	method?: null | #StringMatch @go(Method,*StringMatch) @protobuf(1,bytes,opt)

	// Path What url paths should be mirrored
	// +optional
	path?: null | #StringMatch @go(Path,*StringMatch) @protobuf(2,bytes,opt)

	// Headers What request with matching headers should be mirrored
	// +optional
	headers?: {[string]: #StringMatch} @go(Headers,map[string]StringMatch) @protobuf(3,bytes,opt)
}
...
#StringMatch: {
	...
	exact?: string @go(Exact) @protobuf(1,bytes,opt)
	...
	regex?: string @go(Regex) @protobuf(3,bytes,opt)
}
...
#SetHeaderRoute: {
	...
	// to be included in the `spec.strategy.canary.trafficRouting.managedRoutes` field
	name?: string @go(Name) @protobuf(1,bytes,opt)
	match?: [...#HeaderRoutingMatch] @go(Match,[]HeaderRoutingMatch) @protobuf(2,bytes,rep)
}

#HeaderRoutingMatch: {
	// HeaderName the name of the request header
	headerName: string @go(HeaderName) @protobuf(1,bytes,opt)

	// HeaderValue the value of the header
	headerValue?: null | #StringMatch @go(HeaderValue,*StringMatch) @protobuf(2,bytes,opt)
}
...
#AwsResourceRef: {
	...
	// +optional
	fullName: string @go(FullName) @protobuf(3,bytes,opt)
}
...
#RollbackWindowSpec: {
	revisions?: int32 @go(Revisions) @protobuf(1,varint,opt)
}
